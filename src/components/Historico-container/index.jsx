import style from './historicoContainer.module.css';
...
export const HistoricoContainer = () => {
  const { historico, trechoExibir, setTrechoExibir } = useContext(MotivaContext);

  const [kmPesquisado, setKmPesquisado] = useState("");
  const [trechoPesquisado, setTrechoPesquisado] = useState("");
  const [funcionarioPesquisado, setFuncionarioPesquisado] = useState("");
  ...
  useEffect(() => {
    ...
    const trechoFiltrado = listaHistorico.filter((trecho) => {
      ...
      const matchesTrecho = nomeTrecho
        .toLowerCase()
      ...
      const matchesFuncionario = trecho.funcionario
        .toLowerCase()
        .includes(funcionarioPesquisado.toLowerCase());

      return matchesTrecho && matchesKm && matchesFuncionario;
    });
    ...
    <section className={style.container}>
      <div className={style.divInputs}>
        ...
        <input
          ...
          type="text"
          value={funcionarioPesquisado}
          onChange={(e) => setFuncionarioPesquisado(e.target.value)}
        ></input>
      </div>
        {itensPaginados.length === 0? (
             <p className={style.aviso}>Não há registros históricos no momento.</p>
        ):(
            <>
            <div className={style.headerSolicitacoes}>
                ...
                {itensPaginados.map((historico)=>(
                    <ItemHistorico
                    key={historico.id}
                    nome={historico.nomeTrecho}
                    kmInicial={historico.kmInicial}
                    kmFinal={historico.kmFinal}
                    nomeFuncionario={historico.funcionario}
                    dataServico={formatarData(historico.dataCorte)}
                    tipoVegetacao = {historico.tipoVegetacao}
                    />
                ))}
            </>
        )}
